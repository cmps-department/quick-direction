...
const RequestForm = () => {
  ...
  const categories = useMemo(() => {
    ...
    return data.map(category => ({
      group: category.name,
      items: category.subDirections.map(subCategory => ({
        label: subCategory.name,
        value: subCategory.id?.toString() as string,
      })),
    }));
  }, [data?.length]);
  ...
  const FormSection = useMemo(() => {
    ...
    data?.forEach(category => {
      ...
      subCategory = category.subDirections.find(subCategory => subCategory.id == subDirectionId) as
        | ISubCategory
        | undefined;
    });
